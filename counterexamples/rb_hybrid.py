...
# rules
rules = []
rules.append({'idx': 0, 'from': 0, 'to': 0, 'guard': "(< x (- n (* 2 tb)))"})
rules.append({'idx': 1, 'from': 0, 'to': 2, 'guard': "(>= (+ x y fb) (- n (* 2 tb)))"}) 
...
rules.append({'idx': 8, 'from': 4, 'to': 4, 'guard': "(< x (- n (* 2 tb)))"})
rules.append({'idx': 9, 'from': 4, 'to': 6, 'guard': "(>= (+ x y fb) (- n (* 2 tb)))"})
rules.append({'idx': 10, 'from': 5, 'to': 6, 'guard': "true"})
...
properties.append({'name':"correctness", 'spec':'safety', 'initial':"(= (+ x y) (- n fb))", 'qf':'all', 'reachable':"(< acc (- n fb))"})
