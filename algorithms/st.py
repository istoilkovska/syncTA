...
rules.append({'idx': 1, 'from': 0, 'to': 2, 'guard': "(>= (+ x f) (- n (* 2 t)))"})
...
# properties
properties = []
properties.append({'name':"unforgeability", 'initial':"(= x 0)", 'qf':'some', 'reachable':"(> acc 0)"})
properties.append({'name':"correctness", 'initial':"(= x (- n f))", 'qf':'all', 'reachable':"(< acc (- n f))"})
